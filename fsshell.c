...
int cmd_mv(int argcnt, char *argvec[])
{
	...
	{
		return (fs_moveFile(argvec[1], argvec[2]));
	}
...
int cmd_cp2fs(int argcnt, char *argvec[])
{
	...
	do
	{
		...
		printf("\nReadcnt in Linux: %d \n", readcnt);
		//	printf("Linux buffer: %s \n", buf);
		//	printf("Linux buffer: %s \n", buf);
		b_write(testfs_fd, buf, readcnt);
	} while (readcnt == BUFFERLEN);
	//printf("\nFile LBA index in b_write: %d \n", openFileTable[testfs_fd].fileLBAIndex);
	int size = b_seek(testfs_fd, 0, 3); // Use seek end to get file size
	printf("File size: %d\n", size);
	b_close(testfs_fd);
	close(linux_fd);
	return 0;
...
int main(int argc, char *argv[])
{
	...
	startFileSystem(filename, volumeSize, blockSize);
